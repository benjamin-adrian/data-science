...
for state in states:
    ...
    for action in actions:
        temp[action] = 0.1 # Set to 0.1 if following greedy policy
        temp_e[action] = 0.0
...
def run():
    ...
    for episode_num in range(100):
        ...
        while not World.has_restarted():
            ...
            score += r1
            a2, q_val2 = policy(s2, epsilon) # Change to max_Q(s2) if following Greedy policy
            # Update Q
            learn(s1, a1, r1, s2, a2)
            s1 = s2
            a1 = a2
            steps += 1

        World.restart_game()
        episode = {'episode': episode_num, 'score': score, 'steps': steps, 'alpha': alpha, 'epsilon': epsilon}        #reset_E()
        print(episode)
        log.append(episode)
       # time.sleep(0.01)
        alpha = max(0.1, pow(episode_num+1, -0.4))
        epsilon = max(0.0000001, epsilon*e_decay)

    with open('data/log_eps-greedy_traces_10.csv', 'w') as csvfile:
