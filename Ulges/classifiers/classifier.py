...
class ClassifierSKLearn(Classifier):
    ...
    def __init__(self, model,
        ...
        if os.path.isfile(self.model_file):
            with open(self.model_file, "r") as stream:
                (self.classifier,self.vectorizer,self.int2class) = pickle.load(stream)
    ...
    def train(self, features, labels):
        ...
        class2int = dict(zip(classes,range(len(classes))))
        int2class = dict(zip(range(len(classes)),classes))
        y = [class2int[labels[filename]] for filename in features]
        ...
        with open(self.model_file, "w") as stream:
            pickle.dump((self.classifier,self.vectorizer,int2class), stream)
    ...
    def _inspect_maxent(self):
        ...
        for i,c in self.int2class.items():
            ...
            print 'class', c
            for j,val in best_features[:20]:
                print '   ', terms[j], '%.3f' %(val)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='A text classifier based on SKLEARN.')
    ...
    parser.add_argument('--apply', help="apply the classifier (you'll need to train or load"\
                                        " a trained model before)", action='store_true')
